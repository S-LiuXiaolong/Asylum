...
static void UpdateText()
{
	...
	ss << " }\n\n1 - " << ARRAY_SIZE(curves);
	ss << " - presets  W - wireframe  F - full window  A/D tessellation level";
...
bool InitScene()
{
	...
	if (!GLCreateEffectFromFile("../../../Asset/Shaders/GLSL/simplecolor.vert", 0, 0, "../../../Asset/Shaders/GLSL/drawpoints.geom", "../../../Asset/Shaders/GLSL/simplecolor.frag", &renderpoints)) {
		MYERROR("Could not load point renderer shader");
		return false;
	}

	if (!GLCreateEffectFromFile("../../../Asset/Shaders/GLSL/simplecolor.vert", 0, 0, "../../../Asset/Shaders/GLSL/drawlines.geom", "../../../Asset/Shaders/GLSL/simplecolor.frag", &renderlines)) {
	...
	if (!GLCreateComputeProgramFromFile("../../../Asset/Shaders/GLSL/tessellatecurve.comp", &tessellatecurve)) {
		MYERROR("Could not load compute shader");
		return false;
	}

	if (!GLCreateComputeProgramFromFile("../../../Asset/Shaders/GLSL/tessellatesurface.comp", &tessellatesurface)) {
		MYERROR("Could not load compute shader");
		return false;
	}
